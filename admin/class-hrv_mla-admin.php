...
class HRV_MLA_Admin {
	...
	public function add_booking_column() {
		add_filter(
			...
			function ( $columns ) {
				unset( $columns['date'] );
				$columns['property_name']        = __( 'Property Name', 'hrv_mla' );
	...
	public function hrv_mla_booking_days( $column_name, $post_id ) {
		...
		if ( 'property_name' == $column_name ) {
			echo get_field( 'property', $post_id );
		}
	...
	public function send_hrv_email( $to, $subject, $content ) {
		...
		if ( get_field( 'testing', 'option' ) ) {
			...
			$toAll = array($to, get_field( 'admin_email', 'option' ));
			wp_mail( $toAll, $subject, $content, $headers );
		}
	...
	public function ciirus_get_property_rates_old( $id, $checkin, $nights ) {
		...
		if ( $err ) {
			...
			$rates          = isset( $arr_output['Rate'] ) ? $arr_output['Rate'] : 0;
...
$response = preg_replace( '/(<\ /?)(\w+):([^>]*>)/', '$1$2$3', $response );
	...
	public function ciirus_calculated_booking_price_v2( $id, $checkin, $checkout ) {
		...
		$price['commission'] = $additional_price;
    ...
    return isset( $propertyTaxRatesApi['total_rates'] ) ? $propertyTaxRatesApi['total_rates'] : 0;
    }
    ...
    public function get_season_total_price($id, $checkin, $nights, $price_cat_ID) {


		$totalPrice = 0; // Initialize total price
		$checkinDateTime = new DateTime($checkin); // Assuming $checkin is in a suitable format for DateTime
		...
		for ($i = 0; $i < $nights; $i++) {
			...
			$totalPrice += $price_with_owner;
		}

		return $totalPrice; // Return the total price for all nights
	}
	...
	public function send_ask_payment_email_function() {
		...
		while ( $query->have_posts() ) :
			...
			$yesterdayTimestamp = strtotime('-1 day', strtotime($today));

			$yesterday = date('d/m/Y', $yesterdayTimestamp);

			$due_date = get_field('due_date');

			if ($due_date === $yesterday && 'full' !== get_field('payment_status')) {

				if ( get_post_meta( get_the_ID(), 'reminder_email_sent', true ) != 'yes' ) {

					$this->send_reminder_email_process( get_the_ID(), get_field( 'email' ) );
	...
	public function send_reminder_email_process( $post_id, $to_email ) {
		...
		if ( get_field('api_price') ) {
			...
			$email_to_send = str_replace( 'TOTAL_ROOM_RATE',  number_format(floatval(get_field('total_ciirus_price_with_comission')), 2), $email_to_send );
			$total_price   = get_field( 'total_price_and_extras' );

		} else {
			$email_to_send = str_replace( '[BOOKING_DETAILS]', $this->booking_details_content(), $email_to_send );
			$email_to_send = str_replace( 'HOME_RENTAL_PRICE',  number_format( floatval(get_field('booking_season_price')), 2), $email_to_send  );
			$email_to_send = str_replace( 'TOTAL_PRICE',  number_format( floatval(get_field('total_price')), 2), $email_to_send );
		}
		...
		if ( have_rows( 'golf_booking' ) ) {
			$golf_bookings .= '<div style="overflow-x:auto;" class="golf-table"><table style="border: 1px solid #000; border-collapse: collapse;" class="golf"><thead>
						...
						<th valign="top" align="center" style="border: 1px solid #000; border-collapse: collapse; padding: 5px;">Actual Time</th>
			 ...
			 while ( have_rows( 'golf_booking' ) ) {
				...
				<td valign="middle" align="center" style="border: 1px solid #000; border-collapse: collapse; padding: 5px;">' . get_sub_field( 'actual_time' ) . '</td>
		...
		if ( $total_golf_booking ) {
			$golf_content = '<tr class="item">
                    <td>Golf Fees</td>
                    <td>$'.number_format( floatval($total_golf_booking), 2).'</td>
                </tr>';
		...
		if ( have_rows( 'extra_cost' ) ) :
			...
			<td>$' . (get_sub_field( 'price' ) ? number_format( floatval(get_sub_field( 'price' )), 2) : '0.00') . '</td>
			</tr>';
			endwhile;
		endif;

		$ref_id = get_field( 'old_booking_id' ) ? get_field( 'old_booking_id' ) : get_the_ID();

		$email_subject = $this->get_reminder_email_subject() ? $this->get_reminder_email_subject() : 'Thanks for enquiring';
		$email_subject = str_replace( '[REF_#]', 'HRV-' . $ref_id, $email_subject );
		$email_subject = str_replace( '[GUEST_NAME]', get_field( 'first_name' ) . ' ' . get_field( 'surname' ), $email_subject );

		$email_to_send = str_replace( 'BOOKING_ID', 'HRV-' . $ref_id, $email_to_send );
		...
		$email_to_send = str_replace( 'PROPERTY_ADDRESS', get_field( 'address', $property_id ), $email_to_send );
		$email_to_send = str_replace( 'TOTAL_PRICE',  number_format(floatval($total_price), 2), $email_to_send );
		$email_to_send = str_replace( 'RENT_PRICE',  number_format(floatval(get_field( 'booking_season_price' )), 2), $email_to_send );
		$email_to_send = str_replace( 'TOTAL_BALANCE',  number_format(floatval(get_field('balance')), 2), $email_to_send );
		...
		$email_to_send = str_replace( 'TOTAL_DEPOSIT',  number_format( floatval(get_field( 'total_amount_paid' )), 2), $email_to_send );
		$email_to_send = str_replace( 'AMOUNT_PAID',  number_format( floatval(get_field( 'balance' )), 2), $email_to_send );
				...
				if ( get_post_meta( get_the_ID(), 'reminder_email_sent', true ) != 'yes' ) {
					...
					if ( have_rows( 'extra_cost' ) ) :
						while ( have_rows( 'extra_cost' ) ) :
							...
							$other_addons .= '<tr>
						<td>' . get_sub_field( 'extra_cost' ) . '</td>
						<td colspan="2" align="right">$' . (get_sub_field( 'price' ) ? number_format(floatval(get_sub_field( 'price' )),2) : '0.00') . '</td>
						</tr>';
						endwhile;
					endif;
					...
					$booking_id  = 'HRV-'. get_field( 'old_booking_id' ) ? get_field( 'old_booking_id' ) : get_the_ID();



					$request_payment_email_content = str_replace( 'BOOKING_ID', $booking_id, $request_payment_email_content );
	...
	public function calculate_deposit( $post_id ) {
		if ( 'bookings' == get_post_type( $post_id ) ) {
			...
			if ( get_field('api_price', $post_id) ) {
				$total_price = get_field( 'total_price_and_extras', $post_id );
			} else {
	...
	public function get_date_price( $date, $price_category ) {
		...
		$ownerbookingpercent = get_field( 'property_owner_booking_percentage', get_the_ID() ) ? get_field( 'property_owner_booking_percentage', get_the_ID() ) : get_field( 'additional_pricing', 'option' )['default_property_owner_booking_percentage'];
        ...
        if ( (int)$dateToCheck >= (int)$rangeStart && (int)$dateToCheck <= (int)$rangeEnd ) {
			...
			$cat= get_sub_field( 'categories' );
				if ( $cat == $price_category ) {
					$day_price = get_sub_field( 'price' );
					$owner_price = 0;
					if ( $ownerbookingpercent ) {
						$owner_price = ( $ownerbookingpercent / 100 ) * $day_price;
					}
					$price = $owner_price + $day_price;
			...
			if ( ! get_field( 'api_price', $post_id ) ) {
				...
				if ( $extra_costs ) {
					foreach ( $extra_costs as $cost ) {
						$total_all = floatval($total_all) + floatval($cost['price']);
					}
				}
	...
	if ( 'bookings' == get_post_type( $post_id ) && ( 'no' !== get_post_meta( $post_id, 'manual_booking', true ) ) ) {
		...
		foreach ( $extra_cost as $cost ) {
			...
			$total_extra_owner = (float)$total_extra_owner + (float)$owner_price;
		...
		if ( get_field( 'api_price', $post_id ) ) {
			...
			if ( $extra_costs ) {
				foreach ( $extra_costs as $cost ) {
					$total_all = $total_all + (float)$cost['price'];
					$owner_total = $owner_total + (float)$cost['owner_price'];
				}
			}
			...
			if ( $golf_bookings ) {
				foreach ( $golf_bookings as $golf ) {
					$total_all = $total_all + (float)$golf['price'];
				}
			}

			$total_price_and_extras = $total_all + (float)$total_ciirus_price_with_comission;

			$total_owner_amount = $owner_total + (float)get_field('ciirus_room_price', $post_id);
			...
			update_field('api_profit', (float)$total_price_and_extras - (float)$total_owner_amount, $post_id );
			...
			$owner_total_price = (float)$owner_total_villa_price + (float)$total_extra_owner;



			$total_price = get_field('total_price', $post_id);
			if ( get_field('total_profit', $post_id) === '') {
				update_field('total_profit', (float)$total_price - (float)$owner_total_price, $post_id);
			}

			update_field('owner_total_extra_price', (float)$total_extra_owner, $post_id);

			if (get_field('owner_total_price', $post_id) === '' ) {
				update_field('owner_total_price', ( (float)get_field('owner_total_villa_price', $post_id) + (float)get_field('owner_total_extra_price', $post_id) ), $post_id);
			}
	...
	public function update_specific_check_in_instructions( $post_id ) {
		if ( 'bookings' == get_post_type( $post_id ) ) {

			$property_id = get_field('property_post', $post_id)[0];

			$specific_check_in_instructions = get_field('specific_check_in_instructions', $property_id);
			$alarm_information = get_field('alarm_information', $property_id);
			$check_in_instructions = get_field('check_in_instructions', $property_id);
			$directions_from_airport = get_field('directions_from_airport', $property_id);

			if ( get_field( 'specific_check_in_instructions', $post_id) === '' && $specific_check_in_instructions) {
				update_field( 'specific_check_in_instructions', $specific_check_in_instructions, $post_id);
			}

			if ( get_field( 'alarm_information', $post_id) === '' && $alarm_information) {
				update_field( 'alarm_information', $alarm_information, $post_id);
			}

			if ( get_field( 'check_in_instructions', $post_id) === '' && $check_in_instructions) {
				update_field( 'check_in_instructions', $check_in_instructions, $post_id);
			}

			if ( get_field( 'directions_from_airport', $post_id) === '' && $directions_from_airport) {
				update_field( 'directions_from_airport', $directions_from_airport, $post_id);
			}
	...
	public function owner_manual_email_process( $post_id ) {
		...
		$owner_name = get_field( 'booking_property_owner_name' );
		$today         = time();
		$diff          = $arrival_date - $today;
		$days          = floor( $diff / ( 60 * 60 * 24 ) );
		...
		if ( have_rows( 'extra_cost' ) ) :
			while ( have_rows( 'extra_cost' ) ) :
				...
				$other_addons .= '<tr class="item">
			<td>' . get_sub_field( 'extra_cost' ) . '</td>
			<td>$' .  number_format(floatval(get_sub_field( 'owner_price' )),2) . '</td>
			</tr>';
			endwhile;
		endif;
		...
		if ( $api_price == 1 ) {
			...
			$email_to_owner = str_replace( 'HOME_RENTAL_PRICE',  number_format(floatval(get_field('ciirus_room_price')),2), $email_to_owner );
			$email_to_owner = str_replace( 'TOTAL_PRICE',  number_format(floatval(get_field('total_payment_to_owner')),2), $email_to_owner );
		} else {
			...
			$email_to_owner = str_replace( 'HOME_RENTAL_PRICE',  number_format(floatval(get_field('owner_total_villa_price')),2), $email_to_owner  );
			$email_to_owner = str_replace( 'TOTAL_PRICE',  number_format(floatval(get_field('owner_total_price')),2), $email_to_owner );
		}

		$ref_id = get_field( 'old_booking_id' ) ? get_field( 'old_booking_id' ) : get_the_ID();

		$email_to_owner = str_replace( 'BOOKING_ID', 'HRV-' . $ref_id, $email_to_owner );
		...
		$owner_email_subject = str_replace( '[REF_#]', 'HRV-' . $ref_id, $owner_email_subject );
		$owner_email_subject = str_replace( '[GUEST_NAME]', get_field( 'first_name' ) . ' ' . get_field( 'surname' ), $owner_email_subject );
	...
	public function full_paid_process( $post_id ) {
		...
		$diff          = $arrival_date - $today;
		$days          = floor( $diff / ( 60 * 60 * 24 ) );
		$total_price   =  number_format( floatval(get_field( 'total_price' )), 2);
		$deposit_price = $total_price - ( $total_price * 0.10 );
		$deposit_price = $deposit_price > $this->deposit ? $this->deposit : $deposit_price;
		...
		if ( get_field('api_price') ) {
			...
			$email_to_send = str_replace( 'TOTAL_ROOM_RATE', number_format(floatval(get_field('total_ciirus_price_with_comission')),2), $email_to_send );
			$total_price   =  number_format( floatval(get_field( 'total_price_and_extras' )), 2);
		...
		$directions = '';
		if ( get_field( 'directions_from_airport', $property_id ) || get_field( 'directions_from_airport', $post_id ) ) {
			ob_start();
			$dir_airport = get_field( 'directions_from_airport', $property_id );
			if ( get_field( 'directions_from_airport', $post_id ) ) {
				$dir_airport = get_field( 'directions_from_airport', $post_id );
			}
			?>
                <tr class="heading">
                ...
                <tr class="item">
                    <td colspan="2"><?php echo $dir_airport; ?></td>
                </tr>
                <?php
		...
		$checkin = '';
		if ( get_field( 'check_in_instructions', $property_id ) || get_field( 'check_in_instructions', $post_id ) ) {
			ob_start();
			$checkin_ins = get_field( 'check_in_instructions', $property_id );
			if ( get_field( 'check_in_instructions', $post_id ) ) {
				$checkin_ins = get_field( 'check_in_instructions', $post_id );
			}
			?>
                <tr class="heading">
                ...
                <tr class="item">
                    <td colspan="2"><?php echo $checkin_ins; ?></td>
                </tr>
                <?php
		...
		$specific_checkin = '';
		if (  get_field( 'specific_check_in_instructions', $property_id ) || get_field( 'specific_check_in_instructions', $post_id )) {
			ob_start();
			$specific = get_field( 'specific_check_in_instructions', $property_id );
			if ( get_field( 'specific_check_in_instructions', $post_id ) ) {
				$specific = get_field( 'specific_check_in_instructions', $property_id );
			}
			?>
                <tr class="heading">
                ...
                <tr class="item">
                    <td colspan="2"><?php echo $specific; ?></td>
                </tr>
                <?php
		...
		$alarm = '';
		if ( get_field( 'alarm_information', $property_id ) || get_field( 'alarm_information', $post_id ) ) {
			ob_start();
			$alarm = get_field( 'alarm_information', $property_id );
			if ( get_field( 'alarm_information', $post_id ) ) {
				$alarm = get_field( 'alarm_information', $post_id );
			}
			?>
                <tr class="heading">
                ...
                <tr class="item">
                    <td colspan="2"><?php echo $alarm; ?></td>
                </tr>
                <?php
			$alarm = ob_get_clean();
		}
		...
		if ( have_rows( 'golf_booking') ) {
			$golf_bookings .= '<tr class="heading">
                <td colspan="2">GOLF BOOKING DETAILS</td>
            </tr><tr class="item">
                <td colspan="2">';
			$golf_bookings .= '<div style="overflow-x:auto;" class="golf-table"><table style="border: 1px solid #000; border-collapse: collapse;" class="golf"><thead>
						<tr>
			 ...
			 $golf_bookings .= '</tbody></table></div></td></tr>';
	...
	public function booking_golf_email_metabox_callback() {
		...
		?>
                ...
                <div id="booking_golf_email_wrap">
                    ...
                    <button id="booking_email_btn" class="button button-primary button-large"
                        style="margin-top: 5px;">Send
                        Golf
                        Booking
                        Email</button>
                </div>
	...
	public function send_golf_booking_email() {
		 ...
		 if ( have_rows( 'golf_booking', $_POST['post_id'] ) ) {
			$golf_bookings .= '<div style="overflow-x:auto;" class="golf-table"><table class="golf"><thead>
						...
						<th valign="top" align="center">Actual Time</th>
			 ...
			 while ( have_rows( 'golf_booking', $_POST['post_id'] ) ) {
				...
				<td valign="middle" align="center">' . get_sub_field( 'actual_time' ) . '</td>
