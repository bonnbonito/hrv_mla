...
class HRV_MLA_Public {
	...
	public function check_availability() {
        ...
        if ( get_field('api_price', $property_id) ) {
			...
			$api_get_price = $hrv_admin->ciirus_calculated_booking_price_v2( $id, $checkin, $checkout );
            $results['price']       =  round( $api_get_price['total_with_comission'] );
        } else {
            ...
            $results['price_category'] =  $price_category;
            $results['season'] = $hrv_admin->get_season( $checkin );
            $results['owner_percent'] = get_field( 'property_owner_booking_percentage', $property_id ) ? get_field( 'property_owner_booking_percentage', $property_id ) : get_field( 'additional_pricing', 'option' )['default_property_owner_booking_percentage'];
...
<?php
	...
	public function property_available() {
		...
		if ( get_field( 'api_price', $id ) ) {
			...
			$price['season'] = $hrv_admin->get_season( $checkin );
			$price['total']  = $hrv_admin->get_season_total_price( $id, $checkin, $nights, $price_cat_ID);
			$price['price_category'] = $price_cat_ID;
			$price['nights'] = $nights;
			$price['checkin'] = $checkin;
            $price['one_night'] = round($hrv_admin->get_date_price( $checkin, $price_cat_ID), 1);
    ...
    <div class="property-results-content">
		...
		if ( $price ) {
			if ( current_user_can('administrator') ) {
				...
				$hrv_admin   = new HRV_MLA_Admin( 'hrv_mla', HRV_MLA_VERSION );
				$debug_price = $hrv_admin->ciirus_get_property_rates( get_field('ciirus_id', $id), $_GET['date_checkin'], 7 );
				*/
...
<?php
	...
	public function book_property() {
		...
		$state                  = $_POST['state'];
		$zip                    = $_POST['zip'];
		$total_price            = $_POST['totalPrice'] ? round(floatval($_POST['totalPrice']), 2) : 0;
		...
		$due_date               = date( 'd/M/Y', strtotime( $_POST['dueDate'] ) );
		$deposit_compute        = $total_price * 0.10;
		$deposit_price          = $_POST['deposit'] ? round(floatval($_POST['deposit']),2) : 0;
        ...
        if ( $api_price ) {
            ...
            $rental_price  = $bookingprice;
            $owner_rental_price = round($bookingprice - ( $bookingprice * ($ownerbookingpercent/100)), 2);
            $rental_profit = $rental_price - $owner_rental_price;
        }
			...
			if ( $booking_id ) {
				...
				if ( ! empty( $extracostname ) ) {
					foreach ( $extracostname as $key => $cost ) {
                        $owner_price_extra = $extracostoriginalprice[ $key ] * $nights;
                        $commission = $extracostprice[ $key ] - $owner_price_extra;
						$rows[]        = array(
							'extra_cost'       => $cost,
							'price'            => $extracostprice[ $key ],
							'owner_percentage' => $extracostownerpercent[ $key ],
                            'owner_price' => $owner_price_extra,
...
if ( ! $api_price ) {
$owner_total_price = $owner_rental_price + $extra_owner_total;
...
if ( $days_left <= $hrv_admin->days_to_notify ) {
    ...
    //$hrv_admin->send_reminder_email_process( $booking_id, get_field('email', $booking_id) );
    ...
    */
    ...
    $get_to_admin_email_content = str_replace( '[PROFIT]', $rental_profit + $extra_admin_total,
    $get_to_admin_email_content);
    }
    ...
    $get_to_admin_email_content = str_replace( 'GOLF_FEES', '', $get_to_admin_email_content );
    $get_to_admin_email_content = str_replace(
    '[GUEST_NAME]',
    $firstname . ' ' . $surname,
    $get_to_admin_email_content
    ...
    */
    ...
    $email_to_customer = str_replace( 'PROPERTY_ADDRESS', get_field( 'address', $property ), $email_to_customer );
    ...
    $email_to_customer = str_replace( 'GOLF_FEES', '', $email_to_customer );
    $email_to_customer = str_replace( '[GUEST_NAME]', $firstname . ' ' . $surname, $email_to_customer );
    ...
    */
    ...
    $email_to_owner = str_replace( 'PROPERTY_ADDRESS', get_field( 'address', $property ), $email_to_owner );
    if ( $api_price == 1 ) {
    ...
    $email_to_owner = str_replace( 'HOME_RENTAL_PRICE', $owner_rental_price, $email_to_owner );
    }
    $email_to_owner = str_replace( 'RENT_PRICE', $total_room_rate, $email_to_owner );

    $email_to_owner = str_replace( 'GOLF_FEES', '', $email_to_owner );
