...
class HRV_MLA_Public {
	...
	public function get_all_property_details() {
		...
		if ( $query->have_posts() ) :		
			...
			<div class="property-result-wrap">
				...
				<h3 class="property-result-title"><?php echo get_the_title( get_the_ID() ); ?></h3>
			</div>
			<?php
			endwhile;
	...
	public function property_available() {
		...
		<div class="property-result-wrap">
            ...
            <?php } ?>
			<h3 class="property-result-title"><?php echo get_the_title( $id ); ?></h3>
		</div>
		<?php
		$status['content'] = ob_get_clean();
		wp_send_json( $status );
	...
	public function book_property() {
		...
		if ( $payment_intents ) {
			...
			if ( $booking_id ) {
    			...
    			$subscribed = $MailChimp->post("lists/$list_id/members", [
    				'email_address' => $email,
	...
	public function add_shortcodes() {
		...
		add_shortcode(
			...
			function() {
				...
				require_once 'partials/bookingform.php';
		...
		add_shortcode(
			...
			function() {
				...
				require_once 'partials/check-availability.php';
	...
	public function has_no_search_dates() {
		...
		?>
				...
				form.append('bedrooms', <?php echo $_REQUEST['bedrooms']; ?> );
				<?php endif; ?>
