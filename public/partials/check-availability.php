		...
		.then((data) => {
			...
			if (data) {
                ...
                if ( 'available' === data['status'] ) {
                    statusResult.innerHTML = `<h3 style="color: green;">Available</h3> <a href="/book-online/?id=${data['property_id']}&date_checkin=${data['checkin']}&date_checkout=${data['checkout']}&nights=${data['nights']}" class="booknow">Book Now</a>`;
                } else {
