        ...
        <div class="form-container">
            ...
            <div class="custom-form-booking-input-wrapper">
                ...
                <div class="custom-book-form-row-4">
                    ...
                    <input type="hidden" name="ownerRoomPrice" id="ownerRoomPrice"
                        value="<?php echo $bookingprice * ($ownerbookingpercent / 100); ?>">
                    <?php endif; ?>
                    ...
                    <input type="hidden" name="totalRoomRate" id="totalRoomRate" value="<?php echo $total_price; ?>">
                ...
                <div class="extra-cost-input">
					...
					if ( have_rows( 'extra_cost', $_GET['id'] ) ) :
						...
						while ( have_rows( 'extra_cost', $_GET['id'] ) ) :
							...
							$extraCost   = $extraCost + floatval( $price_total );
							// Do something...
							?>
...
(function() {
    ...
    function compute_nights() {
        ...
        noNights.value = days;
        bookingprice.value = parseFloat(days) * parseFloat(seasonprice);
        if (days) {
            getTotalPrice();
        }
    }
    ...
    function getTotalPrice() {
        ...
        const extracosts = document.querySelectorAll('input[name="extra-cost[]"]');
        let total = 0;
        let computedTotal = 0;

        extracosts.forEach(function(cost) {
            if (cost.checked) {
                total += parseFloat(cost.value.replace(/,/g, ''));
            }
        });

        computedTotal = parseFloat(total) + parseFloat(totalRoomRate.value.replace(/,/g, ''));

        let depositCompute = computedTotal * 0.10;

        let depositTotal = depositCompute < HRV.stripe_deposit ? depositCompute
            .toFixed(1) : HRV.stripe_deposit;

        let computed = parseFloat(computedTotal.toFixed(1));
    ...
    function getTotalPrice() {
        ...
        let computedPrice = parseFloat(bookingprice.value);

        console.log('computedPrice', computedPrice);

        let computedTotal = total + parseFloat(computedPrice);

        let depositCompute = computedTotal * .10;

        let depositTotal = (computedTotal * .10) > HRV.stripe_deposit ? HRV.stripe_deposit : (computedTotal * .10)
